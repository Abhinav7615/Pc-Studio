...
export default function Login() {
  ...
  // Check if entered identifier is an admin email
  useEffect(() => {
    ...
    const timer = setTimeout(checkAdminEmail, 300);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result?.ok) {
      ...
      if (isAdminMode) {
        // Admin login - verify role and redirect
        if (session?.user?.role === 'admin' || session?.user?.role === 'staff') {
          router.push('/admin');
        } else {
          setAdminError('You do not have admin access');
          setLoading(false);
        }
      } else {
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label className="block text-gray-900 font-bold mb-1">
              {isAdminMode ? 'Admin Email' : 'Email or Mobile'}
            </label>
            <input
              type="text"
              value={identifier}
              ...
              className={`w-full px-3 py-2 border rounded focus:outline-none text-gray-900 placeholder-gray-400
                ${isAdminMode ? 'border-red-400 focus:border-red-600' : 'border-gray-300 focus:border-blue-600'}
              `}
              required
              placeholder={isAdminMode ? 'Enter admin email' : 'Enter email or mobile'}
            />
            {checkingAdmin && identifier.trim() && (
              <p className="text-xs text-gray-500 mt-1">Checking admin access...</p>
            )}
            {isAdminMode && !checkingAdmin && (
              <p className="text-xs text-red-600 mt-1 font-semibold">🔐 Admin Mode Detected</p>
            )}
          </div>
          <div className="mb-6">
          ...
          <button
            ...
            `}
          >
            {loading ? 'Logging in...' : isAdminMode ? 'Admin Login' : 'Login'}
          </button>
        </form>
