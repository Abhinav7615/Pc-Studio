...
export function generateBotResponse(userMessage: string, history: Array<{ sender: string; message: string }>, botName = 'ShopBot') {
  ...
  const useFriendlyTone = /\b(please|pls|bro|dost|yaar|bhai|sis|friend|friendship|mate|buddy|yaar)\b/.test(lower);
  ...
  const chosen = getRandomItem(fallbackResponses);
  return {
    text: language === 'hi'
